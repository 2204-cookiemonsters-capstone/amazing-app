...
const Profile = ({ navigation }) => {
  ...
  const handleCancelFriendship = async (userid) => {
    try {
      ...
      await deleteDoc(
        doc(firestore, colRef2, where('userid', '==', auth.currentUser.uid))
      );
      alert('Success');
    } catch (error) {
    ...
    <View key={item.userid}>
      <TouchableOpacity style={friendList.friendRow}>
        ...
        <View style={friendList.buttonContainer}>
          ...
          {context == 'Reject' && (
            <TouchableOpacity
              style={friendList.acceptButton}
              onPress={() => handleAcceptFriendship(item.userid)}
            >
              <View style={{ marginLeft: 13, marginRight: 8 }}></View>
              <Text style={{ marginRight: 14 }}>Accept</Text>
            </TouchableOpacity>
          )}
          <TouchableOpacity
            style={friendList.button}
            onPress={() => handleCancelFriendship(item.userid)}
          >
            <View style={{ marginLeft: 13, marginRight: 8 }}></View>
    ...
    <View style={userProfile.container}>
      ...
      <View style={userProfile.body}>
        ...
        <View>
          ...
          <View>
            {incomingFriends.map((item) => friendRow(item, 'Reject'))}
          </View>
