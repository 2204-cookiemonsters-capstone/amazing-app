...
export default function StoriesModal({stories, toggleStoryModal, user}) {
  ...
  return (
    <SafeAreaView  style={styles.container}>
      ...
      <TouchableOpacity
          style={{ position: "absolute", top: windowHeight / 2, right: 10, zIndex: 10 }}
          onPress={() => handleNext()}
        >
          <AntDesign name="right" size={24} color="whitesmoke" />
      </TouchableOpacity>
      <TouchableOpacity
          style={{ position: "absolute", top: windowHeight / 2, left: 10, zIndex: 10 }}
          onPress={() => handlePrevious()}
        >
          <AntDesign name="left" size={24} color="whitesmoke" />
      </TouchableOpacity>
      <View style={{ position: "absolute", top: 50, left: 20, zIndex: 10, flexDirection: 'row' }}>
      ...
      <View style={{ position: "absolute", top: 90, right: 22, zIndex: 10 }}>
        <Text style={styles.text}>{storyIndex + 1}/{stories.length}</Text>
      </View>
      <View>
